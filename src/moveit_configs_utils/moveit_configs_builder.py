...
from .utils import load_file, load_yaml
...
class MoveItConfigsBuilder(object):
    ...
    def __init__(self, robot_name: str, robot_description="robot_description"):
        ...
        self.__package_path = Path(
            get_package_share_directory(robot_name + "_moveit_config")
        )
        setup_assistant_file = self.__package_path / ".setup_assistant"
        if not setup_assistant_file.exists():
            # TODO: Should this throw an exception .?
            logging.warning(
                f"\x1b[33;21mPackage `{self.__package_path}` doesn't have `.setup_assistant` file "
                f"-- using config/{robot_name}.urdf and config/{robot_name}.srdf\x1b[0m"
            )
            self.__urdf_package = self.__package_path
    ...
    def robot_description(self, file_name: Optional[str] = None, mappings: dict = None):
        if file_name is None:
            ...
            robot_description_file = xacro.process_file(
                self.__package_path / "config" / file_name, mappings=mappings
            )
    ...
    def robot_description_semantic(
        ...
        robot_description_semantic = xacro.process_file(
            self.__package_path / (("config/" + file_name) if file_name else self.__srdf_filename),
            mappings=mappings,
        )
    ...
    def robot_description_kinematics(self, file_name: Optional[str] = None):
        self.__moveit_configs.robot_description_kinematics = {
            ...
            + "_kinematics": load_yaml(
                self.__package_path / "config" / (file_name or "kinematics.yaml")
            )
        }
        return self
    ...
    def joint_limits(self, file_name: Optional[str] = None):
        self.__moveit_configs.joint_limits = {
            self.__robot_description
            + "_planning": load_yaml(
                self.__package_path / "config" / (file_name or "joint_limits.yaml")
            )
        }
        return self
    ...
    def planning_pipelines(
        ...
        for pipeline in pipelines:
            self.__moveit_configs.planning_pipelines[pipeline] = load_yaml(
                self.__package_path / "config" / (pipeline + "_planning.yaml")
            )
        return self
